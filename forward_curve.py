...
class IBORDeposit:
    ...
    def bootstrap(self, curve: ForwardCurve) -> None:
        ...
        Direct conversion for short-term deposits
        """
        # For short-term deposits, forward rate equals deposit rate
...
class FRA:
    ...
    def bootstrap(self, forward_curve: ForwardCurve, discount_curve=None) -> None:
        """
        Bootstrap FRA into forward curve
        FRAs directly quote forward rates
        """
        # Check that tenor matches the curve's basis
        tenor = self.end_date - self.start_date
        if abs(tenor - forward_curve.tenor_basis) > 1e-6:
            raise ValueError(f"FRA tenor {tenor} doesn't match curve basis {forward_curve.tenor_basis}")
            
        # Simply add the forward rate
        forward_curve.add_forward_rate(self.start_date, self.rate)
    
    def __repr__(self) -> str:
        return f"{self.index_name}FRA({self.start_date:.2f}-{self.end_date:.2f}, r={self.rate*100:.4f}%)"
